...
// ============================================================
// 1. كتابة api/index.js نظيف بـ LF
// ============================================================
const indexContent = [
...
fs.writeFileSync('api/index.js', indexContent, { encoding: 'utf8' });
console.log('api/index.js written! lines:', indexContent.split('\n').length, 'size:', Buffer.byteLength(indexContent, 'utf8'));

// verify no syntax error
try {
  require('vm').compileFunction(indexContent, [], { parsingContext: require('vm').createContext({}) });
} catch(e) {
  console.log('WARNING syntax check:', e.message);
}

// ============================================================
// 2. إصلاح public/app.js - استعادة التابات + إضافة token
// ============================================================
let app = fs.readFileSync('public/app.js', 'utf8');

// تأكد token functions موجودة
if (!app.includes('getToken')) {
  app = app.replace(
    'const $ = (sel)',
    "const getToken = () => localStorage.getItem('auth_token');\nconst setToken = (t) => { if (t) localStorage.setItem('auth_token', t); };\nconst clearToken = () => localStorage.removeItem('auth_token');\n\nconst $ = (sel)"
  );
  console.log('added token functions');
}

// تأكد apiFetch يرسل token
if (!app.includes("x-auth-token")) {
  app = app.replace(
    'const apiFetch = async (url, options = {}) => {',
    "const apiFetch = async (url, options = {}) => {\n  const token = getToken();\n  if (token) { options.headers = options.headers || {}; options.headers['x-auth-token'] = token; }\n  if (token && !options.headers) options.headers = { 'x-auth-token': token };"
  );
  console.log('added x-auth-token to apiFetch');
}

// تأكد setToken يتم استدعاؤه بعد login/register
if (!app.includes('if (data.token) setToken(data.token)')) {
  app = app.replace(
    'if (!res.ok) {\n    throw new Error(data.error',
    'if (data.token) setToken(data.token);\n  if (!res.ok) {\n    throw new Error(data.error'
  );
  console.log('added auto setToken in apiFetch');
}

// تأكد bootstrap يتحقق من token
if (!app.includes('const token = getToken()')) {
  app = app.replace(
    "try {\n    const { user } = await apiFetch('/api/auth/me');",
    "try {\n    const token = getToken();\n    if (!token) { showAuth(); return; }\n    const { user } = await apiFetch('/api/auth/me');"
  );
  console.log('added token check in bootstrap');
}

// تأكد clearToken في logout
if (!app.includes('clearToken()')) {
  app = app.replace(
    "state.user = null;\n    showAuth();\n  });\n  $('#logoutBtnMobile')",
    "clearToken();\n    state.user = null;\n    showAuth();\n  });\n  $('#logoutBtnMobile')"
  );
  console.log('added clearToken in logout');
}
...
console.log('setToken:', app.includes('setToken'));
console.log('x-auth-token:', app.includes('x-auth-token'));
console.log('clearToken:', app.includes('clearToken'));
console.log('bootstrap token check:', app.includes('const token = getToken()'));

console.log('\nAll done! Now run:');
console.log('git add .');
console.log('git commit -m "fix: clean LF index.js + token auth"');
