...
// ========== Simple Token Auth (no sessions needed) ==========
// نستخدم tokens محفوظة في Supabase بدل sessions
...
const uploadToStorage = async (buffer, filename, folder = 'media') => {
  const { error } = await supabase.storage.from('media').upload(`${folder}/${filename}`, buffer, { upsert: true });
...
const sanitizeText = (value, max = 5000) => { if (typeof value !== 'string') return ''; return value.trim().slice(0, max); };
...
app.get('/api/auth/me', async (req, res) => {
  ...
  if (!session?.users) return res.json({ user: null });
...
app.get('/api/posts', requireAuth, async (req, res) => {
  ...
  const { data: allComments } = await supabase.from('comments').select('*, users(id, display_name, avatar_url, is_owner, is_admin)').in('post_id', (posts || []).map((p) => p.id));
  const commentsMap = {};
  (allComments || []).forEach((c) => { if (!commentsMap[c.post_id]) commentsMap[c.post_id] = []; commentsMap[c.post_id].push({ id: c.id, user_id: c.user_id, text: c.text, created_at: c.created_at, display_name: c.users?.display_name || 'Unknown', avatar_url: c.users?.avatar_url || '', is_admin: c.users?.is_admin || false, is_owner: c.users?.is_owner || false }); });
  const result = (posts || []).map((post) => { const likes = Array.isArray(post.likes) ? post.likes : []; return { ...post, display_name: post.users?.display_name || 'Unknown', avatar_url: post.users?.avatar_url || '', accent_color: post.users?.accent_color || '#63d4ff', is_owner: post.users?.is_owner || false, is_admin: post.users?.is_admin || false, likes_count: likes.length, liked_by_me: likes.includes(req.user.id), comments: commentsMap[post.id] || [], users: undefined }; });
...
app.post('/api/posts/:id/comments', requireAuth, async (req, res) => {
  ...
  res.json({ comment: { ...comment, display_name: author?.display_name || 'Unknown', avatar_url: author?.avatar_url || '', is_admin: author?.is_admin || false, is_owner: author?.is_owner || false } });
});
...
app.get('/api/conversations', requireAuth, async (req, res) => {
  ...
  const { data: memberUsers } = await supabase.from('users').select('id, display_name, avatar_url, accent_color, is_owner, is_admin').in('id', memberUserIds.length ? memberUserIds : [0]);
  const usersMap = {};
  ...
  const result = (conversations || []).map((convo) => {
    ...
    const unread = msgs.filter((m) => { if (m.sender_id === req.user.id) return false; if (!selfMember?.last_seen_at) return true; return m.created_at > selfMember.last_seen_at; }).length;
    const seen = !isGroup && last && last.sender_id === req.user.id && otherMember?.last_seen_at && otherMember.last_seen_at >= last.created_at;
    const displayName = isGroup ? convo.name || otherUsers.map((u) => u.display_name).slice(0, 3).join(', ') || 'Group chat' : otherUser?.display_name || 'Unknown';
    return { id: convo.id, is_group: isGroup, group_name: convo.name || '', member_count: members.length, member_avatars: otherUsers.map((u) => u.avatar_url).filter(Boolean).slice(0, 3), member_names: otherUsers.map((u) => u.display_name).filter(Boolean).slice(0, 3), other_id: isGroup ? 0 : otherUser?.id || 0, display_name: displayName, avatar_url: isGroup ? '' : otherUser?.avatar_url || '', accent_color: isGroup ? '#63d4ff' : otherUser?.accent_color || '#63d4ff', is_admin: isGroup ? false : otherUser?.is_admin || false, is_owner: isGroup ? false : otherUser?.is_owner || false, other_last_seen_at: isGroup ? '' : otherMember?.last_seen_at || '', self_last_seen_at: selfMember?.last_seen_at || '', last_body: last?.body || '', last_media_url: last?.media_url || '', last_media_type: last?.media_type || '', last_created_at: last?.created_at || '', last_sender_id: last?.sender_id || 0, last_sender_name: lastSender?.display_name || '', unread_count: unread, seen: Boolean(seen) };
  });
...
app.post('/api/conversations', requireAuth, async (req, res) => {
  ...
  const { data: sharedMemberships } = await supabase.from('conversation_members').select('conversation_id').eq('user_id', otherId).in('conversation_id', myIds.length ? myIds : [0]);
  for (const m of sharedMemberships || []) { const { data: convo } = await supabase.from('conversations').select('*').eq('id', m.conversation_id).single(); if (convo && !convo.is_group) return res.json({ conversationId: convo.id }); }
...
app.post('/api/conversations/group', requireAuth, async (req, res) => {
  ...
  await supabase.from('conversation_members').insert(unique.map((userId) => ({ conversation_id: convo.id, user_id: userId })));
  res.json({ conversationId: convo.id });
});
app.get('/api/conversations/:id/messages', requireAuth, async (req, res) => {
  ...
  const messages = (msgs || []).map((m) => { const u = sendersMap[m.sender_id]; return { ...m, display_name: u?.display_name || 'Unknown', avatar_url: u?.avatar_url || '', accent_color: u?.accent_color || '#63d4ff', is_admin: u?.is_admin || false, is_owner: u?.is_owner || false }; });
  res.json({ messages, otherLastSeenAt: isGroup ? '' : other?.last_seen_at || '', isGroup, memberCount: (members || []).length });
});
app.post('/api/conversations/:id/messages', requireAuth, uploadMedia.single('media'), async (req, res) => {
  ...
  const { data: user } = await supabase.from('users').select('*').eq('id', req.user.id).single();
  res.json({ message: { ...message, display_name: user?.display_name || 'Unknown', avatar_url: user?.avatar_url || '', accent_color: user?.accent_color || '#63d4ff', is_admin: user?.is_admin || false, is_owner: user?.is_owner || false } });
});
